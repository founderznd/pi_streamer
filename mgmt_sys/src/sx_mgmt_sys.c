...
static void mgmt_rtsp_cback(void *arg, eSX_MGMT_RTSP_EVENT event, uSX_MGMT_RTSP_EVENT_DATA *event_data) {
	...
	if (event == MGMT_RTSP_EVENT_PLAY) {
		...
		mq_send(f_cblk.msg_queue, (char *) &msg, sizeof(sMGMT_SYS_MSG), 0);
	}
...
static void mgmt_sys_thread(void * arg) {
	...
	while (1) {
		bytes_read = mq_receive(f_cblk.msg_queue, (char *) &msg, sizeof(sMGMT_SYS_MSG),
		NULL);
		assert(bytes_read > 0);
		...
		switch (msg.event) {
			case MGMT_SYS_EVENT_PLAY: {
				...
				sx_mgmt_rtp_activate(msg.event_data.play.id, msg.event_data.play.ip, msg.event_data.play.port);
