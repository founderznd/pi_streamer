...
static void session_instance_free(unsigned int index) {
	// TODO: mutex
...
static char * get_session(char *msg) {
...
static char * options_handler(char *msg_rx) {
	unsigned int cseq = get_cseq(msg_rx);

	char * out = malloc(RTSP_BUF_SIZE_MAX);

	snprintf(out,
	RTSP_BUF_SIZE_MAX, "RTSP/1.0 200 OK\r\n"
			"CSeq: %d\r\n"
...
static char * describe_handler(sMGMT_RTSP_SESSION *session, char *msg_rx) {
	char *sdp_str =
			"v=0\r\n"
	...
	sprintf(sdp, sdp_str, session->client_ip_str);
...
static char * setup_handler(sMGMT_RTSP_SESSION *session, char *msg_rx, unsigned short *client_port) {
	unsigned int cseq = get_cseq(msg_rx);
	...
	RTSP_BUF_SIZE_MAX, "RTSP/1.0 200 OK\r\n"
			...
			"Session: %08X\r\n\r\n", cseq, "192.168.1.76", session->client_ip_str, dst_port, dst_port + 1, 62000,
			62000 + 1, 0x11223344);
...
static char * play_handler(char *msg_rx) {
	unsigned int cseq = get_cseq(msg_rx);
	...
	snprintf(out, RTSP_BUF_SIZE_MAX, "RTSP/1.0 200 OK\r\n"
			"CSeq: %d\r\n"
			"%s"
			"Session: %08X\r\n"
			"%s\r\n", cseq, "Range: npt=0.000-\r\n", session,
...
static char * teardown_handler(char *msg_rx) {
	unsigned int cseq = get_cseq(msg_rx);
...
static eRTSP_MSG get_msg_type(char *msg_rx) {
...
static void rtsp_server_thread(void * arg) {
	...
	sprintf(f_cblk.session[id].client_ip_str, "%d.%d.%d.%d", ((unsigned char *) &f_cblk.session[id].client_ip)[0],
			((unsigned char *) &f_cblk.session[id].client_ip)[1], ((unsigned char *) &f_cblk.session[id].client_ip)[2],
	...
	while (1) {
		...
		switch (msg_type) {
			...
			case RTSP_MSG_DESCRIBE: {
				msg_tx = describe_handler(&f_cblk.session[id], msg_rx);
				break;
			}
			case RTSP_MSG_SETUP: {
				msg_tx = setup_handler(&f_cblk.session[id], msg_rx, &client_port);
		...
		if (msg_type == RTSP_MSG_PLAY) {
			...
			f_cblk.user_cback(f_cblk.user_arg, MGMT_RTSP_EVENT_PLAY, &event_data);
		}
		...
		if (msg_type == RTSP_MSG_TEARDOWN) {
			...
			// Callback with event data.
			f_cblk.user_cback(f_cblk.user_arg, MGMT_RTSP_EVENT_TEARDOWN, &event_data);
...
static void rtsp_thread_create(void *arg) {
...
static void tcp_listener_thread(void * arg) {
	...
	// Bind to address.
	int rc = bind(f_cblk.rtsp_sock, (struct sockaddr *) &client_addr, sizeof(client_addr));
	assert(rc == 0);
	...
	while (1) {
		...
		int tcp_sock = accept(f_cblk.rtsp_sock, (struct sockaddr *) &client_addr, &client_addr_len);
