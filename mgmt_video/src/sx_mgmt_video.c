...
static void resources_init(void) {
	...
	f_cblk.msg_queue = mq_open(MGMT_VIDEO_MSG_QUEUE, O_CREAT | O_RDWR, 0644, &queue_attr);
...
// Activate handler. 
void idle_state_activate_event_handler(void) {
	f_cblk.state = MGMT_VIDEO_STATE_ACTIVE;
}

static void active_state_service_event_handler(sMGMT_VIDEO_MSG *msg) {
	sx_queue_push(f_cblk.nal_unit_queue, msg->event_data.service.nal_unit);
}

static void idle_state_service_event_handler(sMGMT_VIDEO_MSG *msg) {
...
static void active_state_reset_event_handler(sMGMT_VIDEO_MSG *msg) {
	sMGMT_VIDEO_NAL_UNIT *nal_unit;
...
static void idle_state_handler(sMGMT_VIDEO_MSG *msg) {
	switch (msg->event) {
...
static void active_state_handler(sMGMT_VIDEO_MSG *msg) {
	switch (msg->event) {
		case MGMT_VIDEO_EVENT_SERVICE:
...
static void mgmt_video_thread(void * arg) {
	sMGMT_VIDEO_MSG msg;
	unsigned int bytes_read;

	while (1) {
		bytes_read = mq_receive(f_cblk.msg_queue, (char *) &msg, sizeof(sMGMT_VIDEO_MSG),
		NULL);
		assert(bytes_read > 0);
...
static void video_capture_thread(void *arg) {
	...
	while (1) {
		...
		mq_send(f_cblk.msg_queue, (char *) &msg, sizeof(sMGMT_VIDEO_MSG), 0);
	}
}

static void mgmt_video_thread_create(void) {
	pthread_create(&f_cblk.thread_id, NULL, (void *) &mgmt_video_thread, NULL);
}

static void video_capture_thread_create(void) {
	pthread_create(&f_cblk.thread_id, NULL, (void *) &video_capture_thread, NULL);
}

void sx_mgmt_video_init(void) {
...
void sx_mgmt_video_activate(void) {
	sMGMT_VIDEO_MSG msg;
...
unsigned char sx_mgmt_video_is_key_frame(sMGMT_VIDEO_NAL_UNIT *nal_unit) {
...
sMGMT_VIDEO_NAL_UNIT * sx_mgmt_video_pps_get(void) {
	sMGMT_VIDEO_NAL_UNIT *nal_unit;
...
sMGMT_VIDEO_NAL_UNIT * sx_mgmt_video_sps_get(void) {
	sMGMT_VIDEO_NAL_UNIT *nal_unit;
...
sMGMT_VIDEO_NAL_UNIT * sx_mgmt_video_get_nal_unit(void) {
	return sx_queue_pull(f_cblk.nal_unit_queue);
}

void sx_mgmt_video_free_nal_unit(sMGMT_VIDEO_NAL_UNIT *nal_unit) {
	free(nal_unit);
}
