...
static void rtp_send(int sock, sSESSION *session, unsigned char *nal_unit, unsigned int nal_unit_len) {
	sRTP_PKT_NODE *head;
	sRTP_PKT_NODE *temp;

	// Get the converted RTP chain.
	head = sx_nal_to_rtp_util_get(session->nal_to_rtp_instance, nal_unit, nal_unit_len);
	temp = head;
	...
	do {
		// Send the packet.
		int rv = sendto(sock, &temp->rtp_pkt, temp->rtp_pkt_len, 0, (struct sockaddr *) &session->peer_addr,
...
static void idle_state_handler(sMGMT_RTP_MSG *msg) {

}

static void activate_handler(sMGMT_RTP_MSG *msg) {
	sSESSION *session;

	logger_log("MGMT_RTP: ACTIVATE received [id = %d]", msg->event_data.activate.id);
...
static void reset_handler(sMGMT_RTP_MSG *msg) {
	logger_log("MGMT_RTP: RESET received [id = %d]", msg->event_data.reset.id);
...
static void service_handler(sMGMT_RTP_MSG *msg) {
	unsigned int i;
	...
	for (i = 0; i < 32; i++) {
		...
		if (session->in_use) {
			while (1) {
				...
				// Send RTP.
				rtp_send(f_cblk.rtp_sock, session, nal_unit_to_send->nal_unit, nal_unit_to_send->nal_unit_len);
...
static void rtp_thread(void * arg) {
	...
	while (1) {
		bytes_read = mq_receive(f_cblk.msg_queue, (char *) &msg, sizeof(msg),
		NULL);
		assert(bytes_read > 0);
...
static void timer_thread() {
	...
	while (1) {
		...
		// Queue message.
		mq_send(f_cblk.msg_queue, (char *) &msg, sizeof(sMGMT_RTP_MSG), 0);
...
void sx_mgmt_rtp_init(void) {
	...
	O_CREAT | O_RDWR, 0644, &queue_attr);

	f_cblk.rtp_sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	assert(f_cblk.rtp_sock != 0);
}

void sx_mgmt_rtp_open(void) {
	// Create RTSP thread.
	rtp_thread_create();
}

void sx_mgmt_rtp_activate(unsigned int id, unsigned int ip, unsigned short port) {
	sMGMT_RTP_MSG msg;
...
void sx_mgmt_rtp_reset(unsigned int id) {
	sMGMT_RTP_MSG msg;
