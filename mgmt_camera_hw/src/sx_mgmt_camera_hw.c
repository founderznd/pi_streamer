...
static int mmal_status_to_int(MMAL_STATUS_T status) {
...
static void encoder_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer) {
	...
	if ((buffer->data[0] == 0x00) && (buffer->data[1] == 0x00) && (buffer->data[2] == 0x00)
			&& (buffer->data[3] == 0x01)) {
		offset = 4;
	}
	...
	memcpy(&hw_buf->nal[index], buffer->data + offset, buffer->length - offset);
...
sSX_CAMERA_HW_BUFFER * sx_camera_hw_get(void) {
	if (f_nal_queue == NULL) {
		return NULL;
	}
